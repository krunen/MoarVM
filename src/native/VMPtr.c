...
MVMREPROps * MVMPtr_initialize(MVMThreadContext *tc) {
    ...
    MVMROOT(tc, st, {
        ...
        tc->instance->VMPtr_WHAT = WHAT;
        MVM_ASSIGN_REF(tc, st, st->WHAT, WHAT);
        st->size = sizeof(MVMPtr);
    });

    MVM_gc_root_add_permanent(tc,
            (MVMCollectable **)&tc->instance->VMPtr_WHAT);
